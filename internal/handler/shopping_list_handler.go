...
func (h *ShoppingListHandler) GetByID(res http.ResponseWriter, req *http.Request) {
	...
	shoppingList, err := h.service.GetByID(req.Context(), id, userID)
	if err != nil {
		if err == domain.ErrNotFound {
			http.Error(res, "Shopping list not found", http.StatusNotFound)
			return
		}
	...
	json.NewEncoder(res).Encode(shoppingList)
}
